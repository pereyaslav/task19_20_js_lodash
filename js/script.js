...
//   result.push(k, v);
// };
var q1 = _.map(json_data, "skills");
// console.log (q1);
q1 = q1.reduce(function (flat, current) {
	return flat.concat(current);
}, []);
console.log('№1_1 Массив скиллов (поле skills) всех людей');
console.log(q1);
q1 = _.uniq(q1);
console.log('№1_2 Исключить повторения');
console.log(q1);
q1 = _.sortBy(q1, function (i) {
	return i.toLowerCase();
});
console.log('№1_3 Сортировка по алфавиту');
console.log(q1);
var q = _.mapValues(json_data, "friends");
var q3 = [];
// for(var key in q) {
//   q3.push(_.map(q[key], "name"));
// };
_.forEach(q, function (value, key) {
	q3.push(_.map(value, "name"))
});
q3 = q3.reduce(function (flat, current) {
	return flat.concat(current);
}, []);
q3 = _.uniq(q3);
console.log('№3 Массив всех друзей всех пользователей, без повторений');
console.log(q3);
var q2 = [];
var s = _.map(json_data, "name");
_.forEach(q, function (value, key) {
	q2.push(_.max(_.map(value, "id")))
});
q2 = _.zip(q2, s);
q2 = _.sortBy(q2);
q2 = q2.reduce(function (flat, current) {
	return flat.concat(current);
}, []);
